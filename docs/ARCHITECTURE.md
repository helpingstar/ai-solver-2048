...
The AiSolver2048 project employs a multi-module architecture to enhance separation of concerns, maintainability, and build efficiency. This architecture distinguishes between application modules, which are user-facing, and library modules, which provide shared functionalities.

### Application modules
These modules represent the entry points and primary user interfaces for the applications:

* **`:app`**: The 2048 Coach application. It integrates the library modules to deliver the on-device AI move-recommendation experience. It is responsible for the application's top-level setup, primary navigation graph, and dependency injection rooting.
...
* **`:core`**: A fundamental library containing shared business logic, essential data models (entities, value objects), domain-specific utilities, and common interfaces. It is designed with minimal or no direct Android framework dependencies to ensure high reusability and testability.
* **`:data`**: Manages the data layer for the application. Responsibilities include defining repositories, data sources (for local persistence like SharedPreferences), data transformation logic (mappers), and managing data synchronization. It abstracts data origin and storage details from other parts of the system.
* **`:ui`**: A library module providing common UI elements, theming resources (colors, typography, shapes), custom Jetpack Compose components, base UI classes, and UI-related utility functions. It ensures a consistent user experience across the application.
...
The lowest level of the data layer are the "data source" classes. These are the raw sources of data that include data persisted in [SharedPreferences](https://developer.android.com/reference/android/content/SharedPreferences) and data retrieved via interactions with [LiteRT](https://ai.google.dev/edge/litert) for on-device inference.

Note that these data sources are constructed in a manner that adheres to a very important principle of the app: **that function calls should not throw exceptions** (see the [style and best practices documentation](STYLE_AND_BEST_PRACTICES.md#best-practices--kotlin) for more details.) In the case of data sources, this tends to mean that suspending functions like those representing inference calls should return a [Result](https://kotlinlang.org/api/latest/jvm/stdlib/kotlin/-result/) type or a custom sealed result class. This is an important responsibility of the data layer as a wrapper around other third party libraries, as dependencies like LiteRT tend to throw exceptions to indicate errors instead.

### Managers

Manager classes represent something of a middle level of the data layer. While some manager classes like [WorkspaceManagerImpl](../app/src/main/java/io/github/helpigstar/aisolver2048/data/workspace/manager/WorkspaceManagerImpl.kt) depend on the lower-level data sources, others may be wrappers around OS-level classes, while others have no dependencies at all. The commonality amongst the manager classes is that they tend to have a single discrete responsibility. These classes may exist solely in the data layer for use inside a repository or manager class, or may be exposed directly to the UI layer.

### Repositories

Repository classes represent the outermost level of the data layer. They can take data sources, managers, and in rare cases even other repositories as dependencies and are meant to be exposed directly to the UI layer. They synthesize data from multiple sources and combine various asynchronous requests as necessary in order to expose data to the UI layer in a more appropriate form. These classes tend to have broad responsibilities that generally cover a major domain of the app, such as onboarding ([OnboardingRepository](../app/src/main/java/io/github/helpigstar/aisolver2048/data/onboarding/repository/OnboardingRepository.kt)) or workspace settings ([WorkspaceSettingsRepository](../app/src/main/java/io/github/helpigstar/aisolver2048/data/workspace/settings/repository/WorkspaceSettingsRepository.kt)).
...
In some cases a source of data may be continuously observed and in these cases a repository may choose to expose a [StateFlow](https://kotlinlang.org/api/kotlinx.coroutines/kotlinx-coroutines-core/kotlinx.coroutines.flow/-state-flow/) that emits data updates using a `DataState` wrapper.

### Note on Dependency Injection

Nearly all classes in the data layer consist of interfaces representing exposed behavior and a corresponding `...Impl` class implementing that interface (ex: [OnboardingDiskSource](../app/src/main/java/io/github/helpigstar/aisolver2048/data/onboarding/datasource/disk/OnboardingDiskSource.kt) / [OnboardingDiskSourceImpl](../app/src/main/java/io/github/helpigstar/aisolver2048/data/onboarding/datasource/disk/OnboardingDiskSourceImpl.kt)). All `...Impl` classes are intended to be manually constructed while their associated interfaces are provided for dependency injection via a [Hilt Module](https://dagger.dev/hilt/modules.html) (ex: [OnboardingRepositoryModule](../app/src/main/java/io/github/helpigstar/aisolver2048/data/onboarding/repository/di/OnboardingRepositoryModule.kt)). This prevents the `...Impl` classes from being injected by accident and allows the interfaces to be easily mocked/faked in tests.
...
The app's approach to MVVM is based around the handling of "state", "actions", and "events" and is encoded in the [BaseViewModel](../ui/src/main/java/io/github/helpigstar/aisolver2048/ui/platform/base/BaseViewModel.kt) class.
...
- Receiving "events" from the associated `ViewModel` and taking the corresponding action, which is typically to trigger a passed-in callback function for navigation purposes. An [EventsEffect](../ui/src/main/java/io/github/helpigstar/aisolver2048/ui/platform/base/util/EventsEffect.kt) should be used to simplify this process.
...
In order to both provide consistency to the app and to simplify the development of new screens, an extensive system of reusable composable components has been developed and may be found in the [components package](../ui/src/main/java/io/github/helpigstar/aisolver2048/ui/platform/components). These tend to bear the prefix `Aisolver...` to easily distinguish them from similar OS-level components. If there is any new unique UI component that is added, it should always be considered if it should be made a shareable component.
...
This is discussed in detail in [Google's state-hoisting documentation](https://developer.android.com/develop/ui/compose/state-hoisting) and can be summarized roughly as follows: **state should be hoisted as high as is necessary to perform any relevant logic and no higher.** This is a pattern that is followed in the AiSolver2048 app and tends to lead to the following:
...
- The VM "events" are consumed using an [EventsEffect](../ui/src/main/java/io/github/helpigstar/aisolver2048/ui/platform/base/util/EventsEffect.kt) and demonstrate how the `onNavigateToNextScreen` may be triggered.
...
- Reusable components (`AisolverLoadingDialog`, `AisolverSwitch`, and `AisolverFilledButton`) are used where possible in order to build the screen using the correct theming and reduce code duplication. When this is not possible (such as when rending the `Text` composable) all colors and styles are pulled from the AisolverTheme object.
...
fun ExampleScreen(
    ...
    Column(
        ...
        Text(
            ...
            style = AisolverTheme.typography.headlineSmall,
            color = AisolverTheme.colorScheme.textColors.primary,
            modifier = Modifier
...
State-based navigation is handled by the [RootNavViewModel](../app/src/main/java/io/github/helpigstar/aisolver2048/ui/platform/feature/rootnav/RootNavViewModel.kt) and [RootNavScreen](../app/src/main/java/io/github/helpigstar/aisolver2048/ui/platform/feature/rootnav/RootNavScreen.kt). The current navigation state is determined within the `RootNavViewModel` by monitoring the overall [UserState](../app/src/main/java/io/github/helpigstar/aisolver2048/data/onboarding/repository/model/UserState.kt). The `UserState` encodes the user's onboarding progress and other top-level flow data. The `RootNavState` calculated by the `RootNavViewModel` is then used by the `RootNavScreen` in order to trigger a navigation only if the overall state has changed.

The benefit of state-based navigation at this level is that it allows for a variety of state changes that may happen deep within the data layer to automatically trigger navigation to the correct top-level flow without the rest of the UI needing to be concerned with these details. For example, when onboarding completes, the data layer can decide the new state and the `RootNavViewModel` can decide where to go next, rather than having the screen that triggered the action choose the next navigation state (which would then need to be redundantly handled in multiple places in the app).
...
At its most basic level, Compose Navigation relies on constructing String-based URIs with a root path to specify a destination and both path and query parameters to handle the passing of required and optional parameters. This process can typically be very error prone. The AiSolver2048 app borrows a pattern best exemplified by the [Now In Android sample project](https://github.com/android/nowinandroid) in order to make this process more type-safe. This consists of:
