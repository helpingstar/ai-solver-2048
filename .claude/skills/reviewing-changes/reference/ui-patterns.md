...
Quick reference for AiSolver2048 Android Compose UI patterns during code reviews. For comprehensive details, read `docs/ARCHITECTURE.md` and `docs/STYLE_AND_BEST_PRACTICES.md`.
...
```kotlin
// ❌ Recreating AisolverButton
Button(
    onClick = onSubmit,
    colors = ButtonDefaults.buttonColors(
        containerColor = AiSolver2048Theme.colorScheme.primary
    )
) {
    Text("Submit")
}
...
- Use AisolverButton, AisolverTextField, etc. for consistency
...
**Key Rules**:
- Use `AiSolver2048Theme.colorScheme` for colors
- Use `AiSolver2048Theme.typography` for text styles
...
### UI Patterns
- [ ] Using existing Aisolver components from `:ui` module?
- [ ] Using AiSolver2048Theme for colors and typography?
