...
**Context**: Refactoring workspace to Repository pattern, but one ViewModel still uses old pattern
...
**Critical Issues:**
- Incomplete migration (app/workspace/WorkspaceViewModel.kt:89)

See inline comments for details.
```

**Inline Comment 1** (on `app/workspace/WorkspaceViewModel.kt:89`):
```markdown
...
This ViewModel still injects WorkspaceManagerImpl directly. Should it use WorkspaceSettingsRepository like the other ViewModels?

\```kotlin
// Current (old pattern)
class WorkspaceViewModel @Inject constructor(
    private val workspaceManager: WorkspaceManagerImpl
)
...
**Inline Comment 2** (on `data/workspace/manager/WorkspaceManagerImpl.kt:1`):
```markdown
...
Can we add @Deprecated to WorkspaceManagerImpl to guide future development?

\```kotlin
@Deprecated(
    message = "Use WorkspaceSettingsRepository interface instead",
    replaceWith = ReplaceWith("WorkspaceSettingsRepository"),
    level = DeprecationLevel.WARNING
)
class WorkspaceManagerImpl @Inject constructor(...)
\```
