...
- Which modules affected? (app, data, ui, core?)
...
- **Module placement**: UI in `:ui`/`:app`, data in `:data`, no circular dependencies
...
Skip minor formatting (spaces, line breaks, etc.).
...
**Key principles:**
- **Ask questions** for design decisions: "Can we use the existing AisolverTextField component here?"
- **Be prescriptive** for clear violations: "Change MutableStateFlow to StateFlow (UDF pattern requirement)"
