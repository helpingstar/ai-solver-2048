...
You are guiding the developer through a comprehensive Android code review for the AiSolver2048 Android project.
...
2. Files changed (count and modules affected)
3. Existing review thread count (PR reviews only — avoids duplicate comments)
...
For PR reviews: offer to post the review to GitHub using `gh pr review <N> --comment -b "$(cat review-summary.md)"` for the summary. For inline comments, use the GitHub API.
